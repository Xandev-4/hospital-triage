...
*(Audit rows are written server-side as a side effect of other state-changing endpoints — never created directly via API.)*
...
- **"Own case" / "case they created"** means ownership is enforced at the **row level**, not just the role level — middleware needs `requireRole('patient')` (or `'receptionist'`) *plus* a query-time filter: `case.patient_id === req.user.id` for patients, `case.created_by === req.user.id` for receptionists.
